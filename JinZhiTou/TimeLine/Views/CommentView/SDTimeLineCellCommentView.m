...
- (void)setupViews
{
    ...
    _likeLabel.font = [UIFont fontWithName:@"Arial" size:14];
...
{
    self->_likeItemsArray = likeItemsArray;
    
    if (self.likeItemsArray && self.likeItemsArray.count > 0) {
        NSString * content = @"";
        for (SDTimeLineCellLikeItemModel * item in self.likeItemsArray) {
            content = [content stringByAppendingFormat:@"%@,",item.userName];
        }
        content = [content substringToIndex:content.length-1];
        _likeLabel.text = content;
    }
}
